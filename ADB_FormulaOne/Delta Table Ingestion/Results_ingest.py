...
df = spark.read.json(f"/mnt/raw/{file_dt}/{source_name}.json", schema=input_schema)
df.display()
...
merge_delta_table("f1_processed", "result_delta_tbl", folder_path, df, merge_condition, "race_id")
...
# MAGIC %sql
# MAGIC select race_id, count(1) from f1_processed.result_delta_tbl
